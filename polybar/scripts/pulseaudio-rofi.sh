...
output_volume() {
     pacmd list-sinks | awk '/name: /{indefault = $2 == "'"<$(get_default_sink)>"'"}
             /muted: / && indefault {muted=$2}
             /volume: / && indefault {volume=$5}
...
input_volume() {
     pacmd list-sources | awk '/name: /{indefault = $2 == "'"<$(get_default_source)>"'"}
             /muted: / && indefault {muted=$2}
             /volume: / && indefault {volume=$5}
             END { print muted=="no"?volume:"Muted" }'
}
